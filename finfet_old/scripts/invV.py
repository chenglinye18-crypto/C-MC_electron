...
def calculate_time(x, y, z, xr, yr, zr, v, name):
  ...
  while not done: 
	...
	t = float(line_list[7]) / float(line_list[1])
	tao += t * v[i][j][k]
        vol += v[i][j][k] * float(line_list[7])
...
for step in steps[1:]:
  ...
  Tstep = calculate_time(x,y,z, xr, yr, zr, v, name)
  T += Tstep
  print 'time of step %s : %E' % (step, Tstep)

print 'the time in channel is : %E' % (T / (len(steps) - 1))
