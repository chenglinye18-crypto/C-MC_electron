...
vg10_noqc_10nm = vg10_noqc_10nm * 100;
vg10_qc_10nm = vg10_qc_10nm * 100;
vg06_qc_10nm = vg06_qc_10nm * 100;
vg06_noqc_10nm = vg06_noqc_10nm * 100;
vg08_qc_10nm = vg08_qc_10nm * 100;
vg08_noqc_10nm = vg08_noqc_10nm * 100;

hold on
h(1)=plot(vd, vg06_noqc_10nm, 'k+-','LineWidth', 6, 'MarkerSize', 12);
h(2)=plot(vd, vg08_noqc_10nm,'ko-','LineWidth', 6, 'MarkerSize', 12);
h(3)=plot(vd, vg10_noqc_10nm,'k^-','LineWidth', 6, 'MarkerSize', 12);

h(4)=plot(vd, vg06_qc_10nm,'k+--','LineWidth', 6, 'MarkerSize', 12);
h(5)=plot(vd, vg08_qc_10nm,'ko--','LineWidth', 6, 'MarkerSize', 12);
h(6)=plot(vd, vg10_qc_10nm,'k^--','LineWidth',6, 'MarkerSize', 12);
...
text(0.35, -7.8e-6, 'Drain Voltage V_d (V)' , 'FontSize', 30);
...
[legh, objh, outh, outm] = legend(h(1:3),'V_g=0.6 V, noqc','V_g=0.8 V, noqc','V_g=1 V, noqc',1);
%set(legh,'Box','off');
