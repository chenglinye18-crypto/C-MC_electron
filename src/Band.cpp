...
{
    /**
     * @brief Calculate velocities within each tetrahedron
     * 利用布里渊区去面基结构计算 速度
     */
    ...
    //..Loop over all tetrahedra 所有的四面体都会走一遍
    // it=四面体[1]
    for (it = 0; it < nt; it++)
    {

        //..build matrix
        // xkk ykk zkk k空间中的向量格点 ()
        /**
         * @brief build matrix
         * @arg xkk, ykk, zkk: k空间中的向量格点
         *      (以每一个面的中心为格点的值)
         */
        ...
        // 这里的mi[x][x]已经是inverse之后的了 (参见群速度的定义(mc模拟资料))
...
/* ------------------------------------------------------- */
/**
 * @brief Calculate the DOS-table and maximum DOS in tet.
 * 
 */
/* ------------------------------------------------------- */
void Band::ZUDI(void){
    ...
    //..first: claculate DOS
    //..loop over all energy-tab-steps
    for (itab = 0; itab <= MTAB; itab++){
        // DO itab=0,MTAB
        eps = energy[itab];
        //..energy index of tetraheda list
        // +1//minim=0; 
        // dlist: 网格的间距
        ie = int(eps * dlist); 
        if (ie < 0)
        ...
        for (ib = 0; ib < nbt; ib++) {
            ...
            for (itl = ptlist[ie][ib]; itl <= ptlist[ie][ib] + ntlist[ie][ib] - 1; itl++){
                ...
                if (eps > eek[tet[0][it]] && eps < eek[tet[3][it]]){
                    ...
                    //**3
                    // ibt: 每个能带的编号 (参见小尺寸蒙特卡罗模拟P447)
                    dos[ibt[it]][itab] += area / vgt[it] * 48.0 / (8.0 * PI * PI * PI); 
                } // 最终求出了对应的态密度                        
            }
        }
        //..end of energy-loop
    }
    //..write dos table to file

    //..cal. maximum dos in tetrahedron
    //..loop over all energy-tab-steps
    // itab=0 is set to 0;
    for (itab = 1; itab <= MTAB; itab++){
        // DO itab=1,MTAB

        //..cal. maximum area for each tetrahedron between the energies epsl and epsu
        ...
        for (it = 0; it < nt; it++)
        {
            ...
            if ((epsu > eek[tet[0][it]]) && (epsl < eek[tet[3][it]])){
                //..the area grows quadratically between the energy of node 1 and 2
                if (epsu <= eek[tet[1][it]])
                    area = FASTSURF(epsu, it);
                //..the area decreases quadratically between the energy of node 3 and 4.
    ...
    //..set maximum area for zero energy
    for (ib = 0; ib < MNB; ib++)
        // DO ib=1,MNB
...
double Band::SURF(double eps, int it)
{
    ...
    /**
     * @brief Calculate the area of the intersection of an equi energy plane with
     *        a tetraherdron
     * 
    ...
    if (eps > eek[tet[0][it]] && eps < eek[tet[3][it]])
    {
        ...
        {
            /*..cal. vectors (relative to origin of k-space) pointing to 
             *  the four nodes of the quadrangle 
             */
            rfac = (eps - eek[tet[0][it]]) / (eek[tet[2][it]] - eek[tet[0][it]]);
            ...
            rfac = (eps - eek[tet[1][it]]) / (eek[tet[2][it]] - eek[tet[1][it]]);
            ...
            rfac = (eps - eek[tet[1][it]]) / (eek[tet[3][it]] - eek[tet[1][it]]);
...
double Band::FII(double x1, double y1, double z1,
    ...
    /**
     * @brief Calculate the square in a triangle
     * @param values of the three vectors pointing to the nodes
     * @arg   The area is given by the absolute value of the cross product
...
void Band::HESSTET(void){
    /**
     * @brief cal. Hessian form for the four surfaces of the tetraheda,
    ...
    for (it = 0; it < nt; it++)
    {
        ...
        datantlin[3][2][it] = datantlin[0][2][it] * xkk[tet[3][it]] 
                             + datantlin[1][2][it] * ykk[tet[3][it]] + datantlin[2][2][it] * zkk[tet[3][it]];
        if (datantlin[0][2][it] * xkk[tet[2][it]] + datantlin[1][2][it] * ykk[tet[2][it]] + datantlin[2][2][it] * zkk[tet[2][it]] < datantlin[3][2][it])
        {
        ...
        datantlin[3][3][it] = datantlin[0][3][it] * xkk[tet[0][it]] + datantlin[1][3][it] * ykk[tet[0][it]] + datantlin[2][3][it] * zkk[tet[0][it]];
        if (datantlin[0][3][it] * xkk[tet[3][it]] + datantlin[1][3][it] * ykk[tet[3][it]] + datantlin[2][3][it] * zkk[tet[3][it]] < datantlin[3][3][it])
        {
...
void Band::SETNEIBSYM(void)
{
    ...
    for (isym = 0; isym < 48; isym++)
    {
        ...
        for (iv = 0; iv <= 4; iv++) /////////becareful
        {
            ...
            ybz[iv][isym] = yk - ykl;
            zbz[iv][isym] = zk - zkl;
            if (fabs(xbz[iv][isym]) < 1e-10)
                xbz[iv][isym] = 0.0;
            if (fabs(ybz[iv][isym]) < 1e-10)
                ybz[iv][isym] = 0.0;
            if (fabs(zbz[iv][isym]) < 1e-10)
                zbz[iv][isym] = 0.0;
...
void Band::CALCFAC(void)
{
    ...
    for (it = 0; it < nt; it++)
    {
        ...
        if (eek[tet[2][it]] > eek[tet[1][it]])
        {
            ...
            facmedium[it] = (faclow[it] * (eps - eek[tet[0][it]]) * (eps - eek[tet[0][it]]) - SURF(eps, it)) / ((eps - eek[tet[1][it]]) * (eps - eek[tet[1][it]]));
        }
        else
            facmedium[it] = 0.0;
    }
    ...
    if (eps > eek[tet[0][it]] && eps < eek[tet[3][it]])
    {
        ...
        else if (eps < eek[tet[2][it]])
        {
            ...
            if (eek[tet[1][it]] - eek[tet[0][it]] > 1e-10)
                rtnv = faclow[it] * (eps - eek[tet[0][it]]) * (eps - eek[tet[0][it]]) - facmedium[it] * (eps - eek[tet[1][it]]) * (eps - eek[tet[1][it]]);
            else
                rtnv = SURF(eps, it);
        }
...
/**
 * @brief READ band structure data from files
 * 
 */
void Band::READBS(void) {
    ...
    if (nband[PELEC] > NBE) {
        cout << "error ETABF: nbe > MNBE";
        exit(0);
        // ENDIF
    }
    if (nband[PHOLE] > NBH)
    {
        cout << "error ETABF: nbh > MNBH";
        exit(0);
        // ENDIF
    }
    if (nband[POXEL] > NBOE)
    ...
    for (it = 0; it < nt; it++)
    {
              ...
              (eek[tet[2][it]] <= eek[tet[3][it]])))
        {
            ...
            cout << "error ETABF: energy order wrong";
            exit(0);
            // ENDIF
        }
        // ENDDO
    ...
    {
        // DO it=1, nt
        xkct[it] = 0.250 * (xkk[tet[0][it]] + xkk[tet[1][it]] + xkk[tet[2][it]] + xkk[tet[3][it]]);
        ykct[it] = 0.250 * (ykk[tet[0][it]] + ykk[tet[1][it]] + ykk[tet[2][it]] + ykk[tet[3][it]]);
        zkct[it] = 0.250 * (zkk[tet[0][it]] + zkk[tet[1][it]] + zkk[tet[2][it]] + zkk[tet[3][it]]);
        // ENDDO
    }
...
void Band::SETMATSYM(void)
{
    ...
    for (isym = 0; isym < 48; isym++)
    {
        ...
        indmat[matsym[3][isym] + 1][matsym[4][isym] + 1][matsym[5][isym] + 1]
              [matsym[0][isym] - 1][matsym[1][isym] - 1][matsym[2][isym] - 1] = isym;
    }
...
/* ---------------------------------------------- */
/** @author rewrite by GangDu DEC.14
 * @brief calculate phonon scattering rates
 * 
 * 
 */
/* ---------------------------------------------- */
void Band::BUILDPHSCATT(void) { //(cscat) 
    ...
    if (jacophfl) {
        //__Bose-Einstein for optical phonons
        ...
        //__optical phonon constants for emission and absorbtion(electrons)
        //     (phonons+II)
        scpre = 14;
        ...
        //__calculate scattering-rates(electrons)
        ipt = PELEC;
        ...
        for (int i = 0; i < 13; i++)
            for (iinital = 0; iinital < nband[ipt]; iinital++)
                for (ifinal = 0; ifinal < nband[ipt]; ifinal++){
                    cout << "scatte[" << i << "][" << ifinal << "][" << iinital << "] = " 
                            << scatte[i][ifinal][iinital] << endl;
                }
    ...
    else if (fiscphfl)
    {
        ...
        kaptemlow = (efaplow * efaplow) * (PI * PI * PI) / efapeet / sirho / (sia0 * sia0) * (1.0 / (exp(efapeet) - 1.0) + 1.0) * 6e0;
        kaptemhigh = (efaphigh * efaphigh) * (PI * PI * PI) / efapeet / sirho / (sia0 * sia0) * (1.0 / (exp(efapeet) - 1.0) + 1.0) * 6e0;

        kaplablow = 35e0 * (efaplow * efaplow) * PI / (efapeel * efapeel) / sirho / (sia0 * sia0) * 2e0;
        kaplabhigh = 35e0 * (efaphigh * efaphigh) * PI / (efapeel * efapeel) / sirho / (sia0 * sia0) * 2e0;

        kaplemlow = (efaplow * efaplow) * (PI * PI * PI) / efapeel / sirho / (sia0 * sia0) * (1.0 / (exp(efapeel) - 1.0) + 1.0) * 6e0 * 2e0;
        kaplemhigh = (efaphigh * efaphigh) * (PI * PI * PI) / efapeel / sirho / (sia0 * sia0) * (1.0 / (exp(efapeel) - 1.0) + 1.0) * 6e0 * 2e0;
        ...
        { // DO iinital =1,nband[ipt]
            ...
            { // DO ifinal  =1,nband[ipt]
                ...
                if (iinital == 0 && ifinal == 0)
                {
                    //_____optical Phonon(Absorption)
                    scatte[0][ifinal][iinital] = kopablow;

                    //_____optical Phonon(Emission)
                    scatte[1][ifinal][iinital] = kopemlow;

                    //_____transversal acoustic Phonon(Absorption)
                    scatte[2][ifinal][iinital] = kaptablow;

                    //_____transversal acoustic Phonon(Emission)
                    scatte[3][ifinal][iinital] = kaptemlow;

                    //_____longitudinal acoustic Phonon(Absorption)
                    scatte[4][ifinal][iinital] = kaplablow;

                    //_____longitudinal acoustic Phonon(Emission)
                    scatte[5][ifinal][iinital] = kaplemlow;
                }
                else
                {

                    //_____transversal optical f-Phonon(Absorption)
                    scatte[0][ifinal][iinital] = kopabhigh;

                    //_____transversal optical f-Phonon(Emission)
    ...
    else
    {
        /*
                 ...
                 STOP
                */
        cout << "error BUILDPHSCAT: Specify phonon system";
        exit(0);
    } // endif
            ...
            { // DO ifinal  =1,nband[ipt]
                ...
                { // DO iscat=1,scpre-1
                    sumscatt[itab][iinital + bandof[ipt]] =
                        sumscatt[itab][iinital + bandof[ipt]] + scatte[iscat][ifinal][iinital] * dose[iscat][ifinal][itab];
                } // ENDDO
                sumscatt[itab][iinital + bandof[ipt]] =
                    sumscatt[itab][iinital + bandof[ipt]] + scattiie[ifinal][iinital][itab];
            } // ENDDO
        }     // ENDDO
    }         // ENDDO
    ...
    { // DO itab=0,MTAB
        ...
        { // DO iinital =1,nband[ipt]
            ...
            { // DO ifinal  =1,nband[ipt]
                ...
                { // DO iscat=1,scprh-1
                    sumscatt[itab][iinital + bandof[ipt]] =
                        sumscatt[itab][iinital + bandof[ipt]] + scatth[iscat][ifinal][iinital] * dosh[iscat][ifinal][itab];
                } // ENDDO
                sumscatt[itab][iinital + bandof[ipt]] =
                    sumscatt[itab][iinital + bandof[ipt]] + scattiih[ifinal][iinital][itab];
            } // ENDDO
        }     // ENDDO
    }         // ENDDO
    ...
    { // DO itab=0,MTAB
        ...
        { // DO iinital =1,nband[ipt]
            ...
            { // DO ifinal  =1,nband[ipt]
                ...
                { // DO iscat=1,scproe
                    sumscatt[itab][iinital + bandof[ipt]] =
                        sumscatt[itab][iinital + bandof[ipt]] + scattoe[iscat][ifinal][iinital] * dosoe[iscat][ifinal][itab];
                } // ENDDO
            }     // ENDDO
        }         // ENDDO
    }             // ENDDO
...
double Band::CALSCATTSUM(double eed, int iinital)
{
    ...
    else
    {
        ...
        rtnv = sumscatt[itab][iinital] + intp * (sumscatt[itab + 1][iinital] - sumscatt[itab][iinital]);
...
void Band::CALSCATTOE(double eed, double *sca, int ib)
{
    ...
    if (eed <= 0e0)
    {
        ...
        { ////DO ifinal=1,nband[POXEL]
            ...
            { // DO is=1,scproe
                sca[is + scproe * (ifinal)] = scattoe[is][ifinal][iinital] * dosoe[is][ifinal][0];
            } // ENDDO
        }     // ENDDO
        //____largest allowed energy
    }
    else if (eed >= emax)
    {
        ...
        { ////DO ifinal=1,nband[POXEL]
            for (is = 0; is < scproe; is++)
            { // DO is=1,scproe
                sca[is + scproe * (ifinal)] = scattoe[is][ifinal][iinital] * dosoe[is][ifinal][MTAB];
            } // ENDDO
        }     // ENDDO
    }
    else
    {
        ...
        { ////DO ifinal=1,nband[POXEL]
            for (is = 0; is < scproe; is++)
            { // DO is=1,scproe
                sca[is + scproe * (ifinal - 1)] = scattoe[is][ifinal][iinital] * (dosoe[is][ifinal][itab] + intp * (dosoe[is][ifinal][itab + 1] - dosoe[is][ifinal][itab]));
            } // ENDDO
        }     // ENDDO
    }         // endif
...
void Band::CALSCATTH(double eed, double *sca, int ib)
{
    ...
    if (eed <= 0.0)
    {
        ...
        { //         DO ifinal=1,nband[PHOLE]
            ...
            {
                sca[is + scprh * (ifinal)] = scatth[is][ifinal][iinital] * dosh[is][ifinal][0];
            } // ENDDO
    ...
    else if (eed >= emax)
    {
        ...
        { //         DO ifinal=1,nband[PHOLE]
            for (is = 0; is < scprh - 1; is++)
            { // DO is=1,scprh-1
                sca[is + scprh * (ifinal)] = scatth[is][ifinal][iinital] * dosh[is][ifinal][MTAB];
            } // ENDDO
    ...
    else
    {
        ...
        { //         DO ifinal=1,nband[PHOLE]
            for (is = 0; is < scprh - 1; is++)
            { // DO is=1,scprh-1
                sca[is + scprh * (ifinal)] = scatth[is][ifinal][iinital] * (dosh[is][ifinal][itab] + intp * (dosh[is][ifinal][itab + 1] - dosh[is][ifinal][itab]));
            } // ENDDO
            sca[scprh - 1 + scprh * (ifinal)] =
                scattiih[ifinal][iinital][itab] + intp * (scattiih[ifinal][iinital][itab + 1] - scattiih[ifinal][iinital][itab]);
        } // ENDDO
    }     // endif
...
void Band::CALSCATTE(double eed, double *sca, int ib)
{
    ...
    if (eed <= 0e0)
    {
        ...
        { // DO ifinal=1,nband[PELEC]
            ...
            { // DO is=1,scpre-1
                sca[is + scpre * (ifinal)] = scatte[is][ifinal][iinital] * dose[is][ifinal][0];
            } // ENDDO
    ...
    else if (eed >= emax)
    {
        ...
        { // DO ifinal=1,nband[PELEC]
            for (is = 0; is < scpre - 1; is++)
            { // DO is=1,scpre-1
                sca[is + scpre * (ifinal)] = scatte[is][ifinal][iinital] * dose[is][ifinal][MTAB];
            } // ENDDO
    ...
    else
    {
        ...
        { // DO ifinal=1,nband[PELEC]
            for (is = 0; is < scpre - 1; is++)
            { // DO is=1,scpre-1
                sca[is + scpre * (ifinal)] = scatte[is][ifinal][iinital] * (dose[is][ifinal][itab] + intp * (dose[is][ifinal][itab + 1] - dose[is][ifinal][itab]));
            } // ENDDO
            sca[scpre - 1 + scpre * (ifinal)] =
                scattiie[ifinal][iinital][itab] + intp * (scattiie[ifinal][iinital][itab + 1] - scattiie[ifinal][iinital][itab]);
        } // ENDDO
    }     // endif
 ...
 * 
 * @param eed - energy of actual electron
 * @param ib  - band index
 * @return double 
 */
/* --------------------------------------------------- */
...
/* --------------------------------------------------------- */
/**
 * @brief interpolate the total DOS between two tab-values
 * 
 * @param eed - energy of actural electron
 * @param ip  - particle
 * 
 * @return double 
 */
...
double Band::CALDOSSUM(double eed, int ip) {
    ...
    else
    {
        itab = (int)(eed / dtable);
        intp = (eed - energy[itab]) / (energy[itab + 1] - energy[itab]);
        rtnv = sumdos[itab][ip] + intp * (sumdos[itab + 1][ip] - sumdos[itab][ip]);
    } // endif
            ...
            { // DO icy=icymin,icymax
                ...
                { // DO icz=iczmin,iczmax
                  //+1 minim=0!!!!!!!!!!!ict[]=ic;ict[] is used as index???????
                    ...
                    //    ..xkl,ykl,zkl
                    d1 = dnt1 * xkl + dnt2 * ykl + dnt3 * zkl - dnt4;
                    d2 = dnt5 * xkl + dnt6 * ykl + dnt7 * zkl - dnt8;
                    d3 = dnt9 * xkl + dnt10 * ykl + dnt11 * zkl - dnt12;
                    d4 = dnt13 * xkl + dnt14 * ykl + dnt15 * zkl - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0)
                    {
                        nct = nct + 1;
                    ...
                    d1 = dnt1 * xkl + dnt2 * ykl + dnt3 * zkh - dnt4;
                    d2 = dnt5 * xkl + dnt6 * ykl + dnt7 * zkh - dnt8;
                    d3 = dnt9 * xkl + dnt10 * ykl + dnt11 * zkh - dnt12;
                    d4 = dnt13 * xkl + dnt14 * ykl + dnt15 * zkh - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0)
                    { // THEN
                        nct = nct + 1;
                    ...
                    //..xkl,ykh,zkl
                    d1 = dnt1 * xkl + dnt2 * ykh + dnt3 * zkl - dnt4;
                    d2 = dnt5 * xkl + dnt6 * ykh + dnt7 * zkl - dnt8;
                    d3 = dnt9 * xkl + dnt10 * ykh + dnt11 * zkl - dnt12;
                    d4 = dnt13 * xkl + dnt14 * ykh + dnt15 * zkl - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0){ 
                        // THEN
                        nct = nct + 1;
                    ...
                    //   ..xkl,ykh,zkh
                    d1 = dnt1 * xkl + dnt2 * ykh + dnt3 * zkh - dnt4;
                    d2 = dnt5 * xkl + dnt6 * ykh + dnt7 * zkh - dnt8;
                    d3 = dnt9 * xkl + dnt10 * ykh + dnt11 * zkh - dnt12;
                    d4 = dnt13 * xkl + dnt14 * ykh + dnt15 * zkh - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0)
                    { // THEN
                        nct = nct + 1;
                    ...
                    //   ..xkh,ykl,zkl
                    d1 = dnt1 * xkh + dnt2 * ykl + dnt3 * zkl - dnt4;
                    d2 = dnt5 * xkh + dnt6 * ykl + dnt7 * zkl - dnt8;
                    d3 = dnt9 * xkh + dnt10 * ykl + dnt11 * zkl - dnt12;
                    d4 = dnt13 * xkh + dnt14 * ykl + dnt15 * zkl - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0)
                    { // THEN
                        nct = nct + 1;
                    ...
                    //   ..xkh,ykl,zkh
                    d1 = dnt1 * xkh + dnt2 * ykl + dnt3 * zkh - dnt4;
                    d2 = dnt5 * xkh + dnt6 * ykl + dnt7 * zkh - dnt8;
                    d3 = dnt9 * xkh + dnt10 * ykl + dnt11 * zkh - dnt12;
                    d4 = dnt13 * xkh + dnt14 * ykl + dnt15 * zkh - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0)
                    { // THEN
                        nct = nct + 1;
                    ...
                    //   ..xkh,ykh,zkl
                    d1 = dnt1 * xkh + dnt2 * ykh + dnt3 * zkl - dnt4;
                    d2 = dnt5 * xkh + dnt6 * ykh + dnt7 * zkl - dnt8;
                    d3 = dnt9 * xkh + dnt10 * ykh + dnt11 * zkl - dnt12;
                    d4 = dnt13 * xkh + dnt14 * ykh + dnt15 * zkl - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0)
                    { // THEN
                        nct = nct + 1;
                    ...
                    //   ..xkh,ykh,zkh
                    d1 = dnt1 * xkh + dnt2 * ykh + dnt3 * zkh - dnt4;
                    d2 = dnt5 * xkh + dnt6 * ykh + dnt7 * zkh - dnt8;
                    d3 = dnt9 * xkh + dnt10 * ykh + dnt11 * zkh - dnt12;
                    d4 = dnt13 * xkl + dnt14 * ykh + dnt15 * zkh - dnt16;
                    if (d1 > 0.0 && d2 > 0.0 &&
                        d3 > 0.0 && d4 > 0.0)
                    { // THEN
...
/**
 * @brief Calculate intrinsic carrier density
 *      (DOS is only for one spin directions)
 * 
 */
void Band::GETINDENS(void) {
    ...
    for (itab = 1; itab <= MTAB; itab++) {
        ...
        en0 = en0 + ((doslmo * eel - dosl * eelmo) * (exp(-eelmo) - exp(-eel)) 
                        + (dosl - doslmo) * ((eelmo + 1e0) * exp(-eelmo) 
                        - (eel + 1e0) * exp(-eel))) / (eel - eelmo);
        // hole
        dosl = CALDOSSUM(eel, PHOLE);
        doslmo = CALDOSSUM(eelmo, PHOLE);
        ep0 = ep0 + ((doslmo * eel - dosl * eelmo) * (exp(-eelmo) - exp(-eel)) 
                        + (dosl - doslmo) * ((eelmo + 1e0) * exp(-eelmo) 
                        - (eel + 1e0) * exp(-eel))) / (eel - eelmo);
...
/* --------------------------------- */
/**
 * @brief GET the density of states
 * 
 * @param calcdos 
 */
/* --------------------------------- */
void Band::GETDOS(bool calcdos)
{
    ...
    if (itab != MTAB) {
        ...
        cout << "error ETABF: wrong number of table entries in file";
        exit(0);
    }
    // read dos
    for (itab = 0; itab <= MTAB; itab++)
    ...
    //__normalize density of states and maximum areas of intersection
    for (iptype = 0; iptype < NumParType; iptype++) {
        // DO iptype=1, NumParType
        DOSMAX[iptype] = 0.0;
        // ENDDO
    ...
    //__calculate maximum DOS in listfp for Fischetti phonon scattering
    for (iband = 0; iband < nbt; iband++)
    {
        // DO iband=1, nbt
...
void Band::BUILDLISTS(void)
{
    ...
    /**
     * @arg ntlist - Number of tetraheda within the energy range
     * @arg ptlist - Pointer to the first tetrahedron in a list
     * @arg  tlist - list of tetraheda 
     * @arg  slist - the same list containing only tetraheda from
     *          the surface of the BZ
     * @arg  zlist - list containing only tetrahedra from
     *          the surface Kz = 0 (used for injection)
     * @arg  clist - list containing tetrahedra which overlap
     *          with a given cube
     */
    // minim=0;maxim=MNB-1
    for (iband = 0; iband < MNB; iband++) {
    ...
    {
        // DO it= 1, nt
        //__Count tetraheda per energy range
        //__get minimum and maximum energy index within the tetrahedron
        //  + 1;//minim=0;
    ...
    for (iband = 0; iband < nbt; iband++)
    {
        ...
        for (itab = 0; itab < MWLES; itab++)
        {
            ...
            if (ipoinss >= MWLIS)
            {
                ...
                cout << "error ETABF: ipoinss > MWLIS";
                exit(0);
            }
        }
    }
    ...
    for (iband = 0; iband < nbt; iband++)
    {
        ...
        for (itab = 0; itab < MWLEFP; itab++)
        {
            ...
            if (ipointfpab >= MWLIFP)
            {
                ...
                cout << "error ETABF: ipointfpab > MWLIFP";
                exit(0);
                // ENDIF
            }
            ...
            if (ipointfpem >= MWLIFP)
            {
                ...
                cout << "error ETABF: ipointfpem > MWLIFP";
                exit(0);
            }
        }
    }
...
/* ------------------------------------------------ */
/**
 * @brief process elec. scatt. command
 * 
 * @param path 
 */
/* ------------------------------------------------ */
void Band::IELEC(string path)
{
    ...
    //__set parameters for bandstructure and particle type
    ...
    //__set values for energy discretization [0:MTAB] -> [emin,emax]
    // energy spacing of table
    dtable = 0.0010 / eV0;
    emin = 0.0;
    ...
    for (itab = 0; itab <= MTAB; itab++) { 
        ...
        //cout << "energy[" << itab << "]: " << energy[itab] << endl;
    } // ENDDO

    //__read card parameters
    ...
    bhfl = false;     // true;//lval(12,linum) Brooks-Herring impurity scattering
    ...
    //__normalize phonon temperature and deformation potentials
    // tempt=temperature; df=deformation potantial
    // t=transversal; l=longitudinal; a=acoustic; o=optical; 
    ...
    // df for elastic acoustic phonon scattering (elec)
    ...
    //__parameters for surface scattering

    //__only one impact ionization model can be specified
    i = 0;
    if (kaneiifl)
        i = i + 1;
    if (thomiifl)
    ...
    //__calculate certain band structure values
    if (calcband)
    { // THEN
        /*
        NEIB
    ...
    //__read band structure-values from files
    READBS();

    //__build lists for finding states in k-space
    BUILDLISTS();
    //__get density of states
    GETDOS(calcdos);

    //__build lists for phonon scattering
    BUILDPHSCATT();

    //__get intrinsic carrier density
    GETINDENS();
