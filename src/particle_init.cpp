...
void MeshQuantities::init_particle_data() {
  ...
  if (band.use_analytic_band) {
    debug_out.open("debug_init_particles.txt");
  } else {
          ...
          if (material[C_LINDEX_GHOST_ONE(i,j,k)] == SILICON) 
          {
            ...
            for (ipar = 0; ipar < pnum; ipar ++){ 
              ...
              if (debug_out.is_open()) {
                  ...
                  if (band.use_analytic_band) {
                      band.GetAnalyticV_FromTable(&newpar);
                      vx_out = band.analytic_vx * velo0;
                      vy_out = band.analytic_vy * velo0;
                      vz_out = band.analytic_vz * velo0;
                  } else {
                      band.OutWedge(vx_out, vy_out, vz_out,
                                    band.vt[4 * newpar.itet],
                                    band.vt[4 * newpar.itet + 1],
                                    band.vt[4 * newpar.itet + 2],
                                    newpar.isym);
                      vx_out *= velo0;
                      vy_out *= velo0;
                      vz_out *= velo0;
  ...
  if (band.use_analytic_band && debug_out.is_open()) {
    debug_out.close();
    cout << "  [Info] Initial particle table dumped to 'debug_init_particles.txt'." << endl;
  } else if (debug_out.is_open()) {
    debug_out.close();
    cout << "  [Info] Initial particle table dumped to 'debug_init_particles_fullband.txt'." << endl;
  }
}
